param(
    ...
    [string]$IdentityFile = "D:/.ssh/hostinger_vps",
...
'@
    ...
    $setupScript | ssh -F NUL -p $SshPort -i $IdentityFile $sshTarget "bash -s"
} else {
...
"@
ssh -F NUL -p $SshPort -i $IdentityFile $sshTarget $remoteDeploy
